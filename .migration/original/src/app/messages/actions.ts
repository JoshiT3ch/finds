...
import type { MessagingActionState } from "./action-state";
...
export async function startConversation(
  _previousState: MessagingActionState,
  formData: FormData,
  ...
  const { data, error } = await authenticatedUser.supabase.rpc(
  ...
  if (error || typeof data !== "string" || !UUID_PATTERN.test(data)) {
    ...
    return {
      status: "error",
      message:
        "We could not start this conversation. The listing may no longer be available.",
    };
  }
...
export async function sendMessage(
  ...
  const { data: conversation, error: conversationError } =
    await authenticatedUser.supabase
      ...
      .or(
        `seller_id.eq.${authenticatedUser.userId},buyer_id.eq.${authenticatedUser.userId}`,
      )
      .maybeSingle();

  if (conversationError || !conversation) {
    console.error(
      "Supabase conversation authorization failed.",
      getSafeDatabaseError(conversationError),
    );
    return {
      status: "error",
  ...
  if (messageError) {
    console.error(
      ...
      getSafeDatabaseError(messageError),
    );
    return {
      status: "error",
