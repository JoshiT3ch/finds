...
const MAX_TOTAL_IMAGE_SIZE_BYTES = MAX_LISTING_IMAGES * MAX_IMAGE_SIZE_BYTES;
...
function errorState(message, values, fieldErrors) {
    return {
        status: "error",
        message,
        fieldErrors,
...
function getSubmittedImages(formData) {
    return formData
        .getAll("images")
        .filter((value) => value instanceof File && value.size > 0);
}
function getCoverImageIndex(formData, imageCount) {
...
async function getVerifiedImageExtension(image) {
    const imageExtension = allowedImageTypes[image.type];
    if (!imageExtension) {
        return null;
    }
...
async function validateListingInput(formData) {
    ...
    else if (!PRICE_PATTERN.test(trimmedValues.price) ||
    ...
    else if (submittedImages.reduce((total, image) => total + image.size, 0) >
...
export async function createListing(_previousState, formData) {
    ...
    try {
        ...
        return errorState("Listing creation is unavailable right now.", submittedValues);
    }
    ...
    if (!validation.success) {
        return errorState("Check the highlighted fields.", validation.values, validation.fieldErrors);
    }
    const storagePaths = [];
    const imageUrls = [];
    try {
        for (const image of validation.data.images) {
            ...
            if (uploadError) {
                ...
                return errorState("We could not upload those photos. Check the files and try again.", submittedValues, { image: "We could not upload those photos." });
            }
            ...
            if (!publicUrlData.publicUrl) {
                await supabase.storage
                    .from(LISTING_IMAGES_BUCKET)
                    .remove(storagePaths);
                return errorState("We could not publish the listing right now.", submittedValues);
            }
        ...
        const orderedImageUrls = [
            coverImageUrl,
            ...imageUrls.filter((_imageUrl, index) => index !== validation.data.coverImageIndex),
        ];
        ...
        if (insertError) {
            ...
            return errorState("We could not publish the listing right now. Please try again.", submittedValues);
        }
    }
    catch {
        if (storagePaths.length > 0) {
